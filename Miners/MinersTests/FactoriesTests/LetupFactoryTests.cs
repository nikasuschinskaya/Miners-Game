...
{
    [TestClass]
    public class LetupFactoryTests
    {
        ...
        {
            _texturePath = ConfigurationManager.AppSettings["textureLetup"];
            _x = 0;
            _y = 0;
        }

        [TestMethod]
        public void CreateObject_ReturnsInstanceOfLetupPrize()
        {
            // Arrange
            var factory = new LetupFactory(_x, _y);

            // Act
            var prize = factory.CreateObject();

            // Assert
            Assert.IsInstanceOfType(prize, typeof(Letup));
        }
